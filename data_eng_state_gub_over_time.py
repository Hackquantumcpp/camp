...
import pandas as pd
import datetime
from data_eng_senate import pipeline,  contains_substring, contains_substr_in_list# , margin_rating, margin_with_party
from data_eng_gub import competitive, governor_np
...
def get_state_average_over_time(state):
    ...
    state_race = governor_np[governor_np['state'] == state]# [senate['party'].isin(['DEM', 'REP'])]
    state_pivot = pd.pivot_table(data=state_race, values='pct', index=['poll_id', 'display_name', 'state', 'end_date', 'sample_size',
                                                               'population', 'numeric_grade'], columns=['party_with_cand'],
...
governor_timeseries = get_state_timeseries(competitive['state'].values)